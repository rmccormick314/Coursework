...
public class InvestmentCalculator 
{
   ...
   public void displayResult() 
   {
      ...
      //Rate
      conIO.printString(PIPE_SPACE);
      conIO.printString("Rate of Return", LEFT_COLUMN_WIDTH, "LEFT");
      conIO.printChar(PIPE);
      conIO.printDouble(rate, PRECISION, RIGHT_COLUMN_WIDTH, "RIGHT");
      conIO.printString(SPACE_PIPE);
      conIO.printEndline();
      ...
      //years
      conIO.printString(PIPE_SPACE);
      conIO.printString("Number of years", LEFT_COLUMN_WIDTH, "LEFT");
      conIO.printChar(PIPE);
      conIO.printInt(years, RIGHT_COLUMN_WIDTH, "RIGHT");
      conIO.printString(SPACE_PIPE);
      conIO.printEndline();
   ...
   public void loadInvestmentRecord(File_Output_Class outputFile)
   {
      outputFile.writeDouble(principle, PRECISION);
      outputFile.writeChar(' ');
      outputFile.writeDouble(rate, PRECISION);
      outputFile.writeChar(' ');
      outputFile.writeInt(years);
   }
   
   public boolean retrieveInvestmentRecord(File_Input_Class inputFile)
   {
      boolean success = false;
      
      if (!inputFile.checkForEndOfFile())
         {
            principle = inputFile.getDouble();
            rate = inputFile.getDouble()/100;
            years = inputFile.getInt();
