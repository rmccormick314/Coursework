...
int sysPhyEnd;                              // Largest memory end in use
bool sys_preemption = False;                // system running with preemption?
int sys_quant_cycles;                       // Number of quant cycles
int proc_cycles;                            // time for 1 processor cycle to run
InterruptController sys_inter_controller;   // System interrupt controller
...
PCBDataType *buildSchedule( PCBDataType *PCBlistHead,
                            ...
                            LogNodeType *logListHead )
   {
      // Initialize variables
         ...
         bool no_schedule_found = True;
         bool sched_complete = False;
      ...
      if( scheduleCode == (int)CPU_SCHED_FCFS_N_CODE )
      {
           ...
           // Schedule is found and set
           no_schedule_found = False;
      }

      // First Come First Serve - Preemptive
      if( scheduleCode == (int)CPU_SCHED_FCFS_P_CODE )
      {
           // PCB list comes in as FCFS
           ...
           // With head pointer set, reset working pointer to head
           workingPCBptr = scheduleHeadPtr;
           // Loop over the nodes until end is found
           while( sched_complete != True )
           {
              // If not at end, move on
              if( workingPCBptr->nextNode != NULL )
              {
                 workingPCBptr = workingPCBptr->nextNode;
              }
              // Else must be at the end, tie the list into circular
              else
              {
                 sched_complete = True;
                 workingPCBptr->nextNode = scheduleHeadPtr;
              }
           }
      ...
      if( scheduleCode == (int)CPU_SCHED_RR_P_CODE )
      {
           // PCB list comes in as FCFS
           // Set buffer node to list head
           workingPCBptr = PCBlistHead;

           // Set system preemtion switch
           sys_preemption = True;
           bool sched_complete = False;
           ...
           // Have found and set the schedule
           no_schedule_found = False;
      }
...
bool checkForInterrupt( void )
   {
      for( int iter = 0;
           iter < sizeof( sys_inter_controller.processesComplete );
           iter ++ )
      {
         if( sys_inter_controller.processesComplete[ iter ] == True )
         {
            //printf("got an interrupt on PID #%d\n", iter);
            return True;
         }
      }
...
void *clearProcessMemory( PCBDataType *PCBpointer,
                          MemTableEntryType *memHead )
   {
      ...
      while ( memWorkingNode )
      {
       ...
       // Reset working node to head
       memWorkingNode = memHead;

       // Start scanning again
       while( memWorkingNode )
       {
           // Basically just checking for the physical end of allocated memory
           if( memWorkingNode->physicalAddressEnd > freeMemPhyStart &&
               memWorkingNode != memHead )
              {
                 freeMemPhyStart = memWorkingNode->physicalAddressEnd + 1;
              }
           memWorkingNode = memWorkingNode->nextNode;
       }

       // Set the start of unallocated memory
       memHead->physicalAddressStart = freeMemPhyStart;
...
void initializeInterruptController( void )
   {
      for( int iter = 0;
           ...
           iter ++ )
      {
         sys_inter_controller.processesWaiting[ iter ] = False;
         sys_inter_controller.processesComplete[ iter ] = False;
      }
   }
...
{
   // Initialize variables
      // Pointer for doing work
      PCBDataType *workingPCBptr = head;

      // Pointer for keeping track of previousPCBptrious node
      PCBDataType *previousPCBptr = head;

   // If pointer is null, nothing to do. Return
   if( workingPCBptr == NULL )
   {
      return NULL;
   }

   // If only one node in the list, free it and return
   if( workingPCBptr->nextNode == workingPCBptr )
   {
      clearMetaDataList( workingPCBptr->startingCommand );
      head = NULL;
      free( head );
      return head;
   }
   ...
   workingPCBptr = head;
   while (workingPCBptr != key)
      {
         if (workingPCBptr->nextNode == head)
            {
                break;
            }

         previousPCBptr = workingPCBptr;
         workingPCBptr = workingPCBptr->nextNode;
      }

   if (workingPCBptr == head)
      {
         previousPCBptr = head;
         while (previousPCBptr->nextNode != head)
            {
               previousPCBptr = previousPCBptr->nextNode;
            }
         head = workingPCBptr->nextNode;
         previousPCBptr->nextNode = head;
         clearMetaDataList( workingPCBptr->startingCommand );
         free(workingPCBptr);
      }

   // check if node is last node
   else if (workingPCBptr->nextNode == head && workingPCBptr == head)
      {
          previousPCBptr->nextNode = head;
          clearMetaDataList( workingPCBptr->startingCommand );
          free(workingPCBptr);
      ...
      {
          previousPCBptr->nextNode = workingPCBptr->nextNode;
          clearMetaDataList( workingPCBptr->startingCommand );
          free(workingPCBptr);
      }
...
Function Input/Parameters:  Struct with PID and runtime (InterruptArgType *)
Function Output/Parameters: None
Function Output/Returned:   None
Device Input/Keyboard:      None
Device Output/Monitor:      None
...
void *runInOutOp( void *IOArgs )
   {
      // Initialize variables
      int cycletime;
      struct InterruptArgStruct *args = (struct InterruptArgStruct *)IOArgs;
      char ioarg[256];
      char oparg[256];

      // Cast input var to int pointer
      cycletime = (int) args->runTime;

      // Get the IO arg stuff from args struct
      copyString( oparg, (char *)args->operation );
      copyString( ioarg, (char *)args->ioarg );

      // Put it into an array so it can be logged when interrupts happen
      copyString( sys_inter_controller.processOps[ (int)args->PID ], oparg );
      copyString( sys_inter_controller.processIOArgs[ (int)args->PID ], ioarg );

      // Run timer with referenced var ...
      runTimer( cycletime );
      sys_inter_controller.processesWaiting[ (int) args->PID ] = False;
      sys_inter_controller.processesComplete[ (int) args->PID ] = True;

      // If running with preemption, there will be no join call so exit now
      if( sys_preemption == True )
      {
         pthread_exit( (void *) 0 );
      }
...
void runPCB( PCBDataType *activePCB, char *timer,
             LogNodeType *logListHead, MemTableEntryType *memHead )
   {
      // Initialize variables
         ...
         processDataHead = activePCB->nextCommand;
         ...
         InterruptArgType *ioargs = \
                       (InterruptArgType *)malloc( sizeof( InterruptArgType ) );
         ...
         int remainingQuantCycles = sys_quant_cycles;
         bool recieved_interrupt = False;

      if( sys_inter_controller.processesComplete[activePCB->PID] == True )
      {
         sys_inter_controller.processesComplete[activePCB->PID] = False;
      }

      // Build buffer node's log info for PROCESS SELECTED, TIME REMAINING
      if( activePCB->currentState != (int)RUNNING ||
          activePCB->nextCommand == NULL )
      {
         accessTimer(LAP_TIMER, timer);
         sprintf(bufferString,
             ...
             recieved_interrupt == False )
      {
         ...
         if( compareString( opCommand, "cpu" ) == 0 )
         {
            ...
            // If system is running with preemption,
            if( sys_preemption == True )
            {
               // While we have quantumCycles and CPU stuff to spin,
               while( remainingQuantCycles > 0 &&
                      processDataHead->runTime > 0 &&
                      recieved_interrupt == False )
                  {
                     // Run the timer for 1 CPU cycle time
                     runTimer(proc_cycles);
                     // Subtract time remaining from both PCB and OP node
                     activePCB->timeRemaining -= ( proc_cycles );
                     processDataHead->runTime -= proc_cycles;
                     // Subtract a quantum cycle
                     remainingQuantCycles -= 1;

                     recieved_interrupt = checkForInterrupt();
                     if( recieved_interrupt == True )
                     {
                       accessTimer(LAP_TIMER, timer);
                       interruptPID = getInterruptPID();
                       sprintf(bufferString,
                               "   %s, OS: Interrupted by process %d, %s %s operation\n",
                               timer, interruptPID,
                               sys_inter_controller.processOps[interruptPID],
                               sys_inter_controller.processIOArgs[interruptPID] );
                       copyString( bufferLogNode->logText, bufferString );
                       addLogNode( logListHead, bufferLogNode );

                        if( sys_inter_controller.\
                                    processesWaiting[ activePCB->PID ] == True )
                        {
                           recieved_interrupt = False;
                           sys_inter_controller.\
                                    processesComplete[ activePCB->PID ] = False;
                        }
                     }
                  }
            }

            // If no preemption, just run as normal
            else
            {
               // Run timer for full time
               runTimer( processDataHead->runTime );
               processDataHead->runTime = 0;
            }

            // Special message if process ran out of quant cycles
            if( remainingQuantCycles == 0 )
            {
               // Get time of process end
               accessTimer(LAP_TIMER, timer);
               // Print log to buffer
               sprintf(bufferString,
                       "   %s, OS:%sProcess %d quantum time out, cpu process operation end\n",
            ...
            // If it didn't run out, just proceed as normal
            else
            {
               // LOGGING
               accessTimer(LAP_TIMER, timer);
               ...
               if( recieved_interrupt == True )
               {
                  changePCBstate( activePCB, WAITING, timer, logListHead );
               }
            }
         }
         ...
         if( compareString( opCommand, "dev" ) == 0)
         {
           ...
           // Set args for IO op
           ioargs->runTime = processDataHead->runTime;
           ioargs->PID = activePCB->PID;
           copyString( ioargs->operation, processDataHead->strArg1 );
           copyString( ioargs->ioarg, processDataHead->inOutArg );

           if( sys_preemption == True )
           {
              // For preemption, we need to create a thread attribute
              pthread_attr_init(&attr);
              // and configure it to run detached (no join)
              pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);

              // Create a pthread
              pthread_create( &ioThread, &attr, &runInOutOp,
                              (void *) ioargs );

              // Set the PCB state to blocked
              changePCBstate( activePCB, BLOCKED, timer, logListHead );

              // Set interrupt state
              sys_inter_controller.processesWaiting[ activePCB->PID ] = True;

              // Deduct the time of this process from PCB
              activePCB->timeRemaining -= processDataHead->runTime;
              processDataHead->runTime = 0;
           }
           else
           {
         ...
         // If the process op is done running, move to next
         if( processDataHead->runTime == 0 )
         {
            activePCB->nextCommand = processDataHead->nextNode;
            processDataHead = processDataHead->nextNode;
         }

         // If current quant cycles are exhaused, set waiting state and exit
         if( sys_preemption == True && remainingQuantCycles == 0 )
         {
            changePCBstate( activePCB, WAITING, timer, logListHead );
         }
      }
      ...
      // AND this process doesn't have an IO thread dangling
          ...
          sys_inter_controller.processesWaiting[activePCB->PID] != True)
      {
         ...
         // Clear the sim memory of the PCB
         clearProcessMemory( activePCB, memHead );

         // Show mem clear if mem display is on
         if( memDisplay )
         {
            sprintf( memMessage,
         ...
         if( sys_inter_controller.processesWaiting[ activePCB->PID ] == False )
         {
...
void runSim( ConfigDataType *configPtr,
             OpCodeType *metaDataMstrPtr )
   {
      // INITIALIZE VARIABLES ------------------------------------------------->
         ...
         sys_quant_cycles = configPtr->quantumCycles;
         proc_cycles = configPtr->procCycleRate;
      ...
      sysMemHead = initializeMemory( sysMemHead, logListHead, memAvailable );
      ...
      // Loop while there are PCBs to run
      while( runningPCB != NULL )
      {
         // If running with preemption,
         if( sys_preemption == True )
         {
           // If currently selected PCB is blocked
           if( runningPCB->currentState == (int)BLOCKED )
           {
              PCBDataType *head = runningPCB;
              // Loop over PCB nodes until one is not blocked
              do
              {
                 runningPCB = runningPCB->nextNode;
                 if( runningPCB->PID > processCounter )
                 {
                    runningPCB = runningPCB->nextNode;
                 }
              }
              while ( runningPCB->currentState == (int)BLOCKED &&
                      runningPCB != head );
              ...
              if( runningPCB == head )
              {
                 ...
                 copyString( bufferLogNode->logText, "\n   ");
                 concatenateString( bufferLogNode->logText, systemTimeString );
                 concatenateString( bufferLogNode->logText,
                                    ", OS: All threads blocked, CPU idle" );
                 addLogNode( logListHead, bufferLogNode );
                 ...
                 while( runningPCB->currentState == (int)BLOCKED )
                 {
                    ...
                    if( interrupted == True )
                    {
                       interrupting_pid = getInterruptPID();
                       while( runningPCB->PID != interrupting_pid )
                       {
                          runningPCB = runningPCB->nextNode;
                       }
                       ...
                       sprintf(bufferString,
                               "   %s, OS: Interrupted by process %d %s %s operation\n\n",
                               systemTimeString, interrupting_pid,
                               sys_inter_controller.processOps[interrupting_pid],
                               sys_inter_controller.processIOArgs[interrupting_pid] );
                       copyString( bufferLogNode->logText, bufferString );
                       addLogNode( logListHead, bufferLogNode );

                       changePCBstate( runningPCB,
                                       READY,
                                       systemTimeString,
                                       logListHead );
                    }
           ...
           {
              runningPCB = runningPCB->nextNode;
              if( sys_inter_controller.processesComplete[ runningPCB->PID ] == True )
              {
                 changePCBstate( runningPCB, READY, systemTimeString, logListHead );
              }
           }

           interrupted = checkForInterrupt();

           // If the PCB has exited after it is run,
           if( runningPCB->currentState == (int)EXIT &&
               sys_preemption == True )
           {
              // Remove it from the list
              if( sys_inter_controller.processesWaiting[ runningPCB->PID ] == False &&
                  sys_inter_controller.processesComplete[ runningPCB->PID ] == False )
              {
                 PCBscheduleHead = removePCBNode( PCBscheduleHead, runningPCB );

                 // If it is the last node in the list,
                 if( runningPCB->nextNode == runningPCB )
                    {
                    //    Null & free it
                       runningPCB = NULL;
                       free( runningPCB );
